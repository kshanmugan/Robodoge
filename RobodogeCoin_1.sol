...
contract RoboDogeCoin is Context, IERC20, Ownable {
    ...
    //decline price
    uint256 public _price_target;
    uint256 public _price_percentage_decline;
    ...
    //bool public haltTransfer;

    uint256 public haltEndPeriod;
    ...
    uint256 public rule;//holding percentage 
    ...
    function checkPriceDeclined(uint256 currentPrice) public view returns (bool){
        return currentPrice < (_price_target - _price_target.mul(_price_percentage_decline).div(100));
    }

    function haltTransfers(uint256 haltPeriod,uint256 currentPrice) public onlyOwner{
        require(checkPriceDeclined(currentPrice),"price decline not within percentage");
       // haltTransfer = true;
        haltEndPeriod = haltPeriod + block.timestamp;

    }

    // function resumeTransfers() public onlyOwner{
    //     require(haltEndPeriod < block.timestamp,"Cannot resume before Halt period expires");
    //     haltTransfer = false;
    // }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private isUnlocked(sender) isUnlocked(recipient) {
        ...
        require(block.timestamp > haltEndPeriod,"Price decline halt on transfers");
    ...
    function setPriceTarget (uint256 _new_price_target) public onlyOwner {
        _price_target = _new_price_target;
    }

    function setPriceDeclinePercent(uint256 _new_price_percentage_decline) public onlyOwner{
        _price_percentage_decline = _new_price_percentage_decline;
